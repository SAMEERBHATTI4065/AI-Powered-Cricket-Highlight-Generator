...
const ExportCard = ({ icon, label, format, size, onClick, glowColor }: any) => {
  ...
  return (
    <button
      ...
      style={glowColor && isHovered ? { boxShadow: `0 0 20px ${glowColor}` } : {}}
      className="w-full p-[16px] flex items-center justify-between bg-white/[0.02] border border-white/[0.05] rounded-[16px] transition-all duration-300 group hover:bg-white/[0.05]"
    >
...
const ResultsPage = () => {
  ...
  const sessionId = urlSessionId || localStorage.getItem('last_session_id');
  ...
  useEffect(() => {
    if (!sessionId) {
      setError("No recent analysis found. Please start a new analysis from the Dashboard.");
      setLoading(false);
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-[#05080F] relative selection:bg-primary/30 scroll-smooth">
      ...
      <motion.main
        ...
        className="container mx-auto px-4 sm:px-6 pt-24 sm:pt-32 pb-24 sm:pb-32 max-w-[900px] flex flex-col gap-8 sm:gap-10 text-center"
      >
        ...
        <motion.div
          ...
          className="relative group rounded-2xl md:rounded-[24px] overflow-hidden bg-black aspect-video border-2 border-white/5 shadow-[0_20px_80px_rgba(0,0,0,0.8)] w-full max-w-[480px] sm:max-w-[600px] md:max-w-[680px] lg:max-w-[720px] mx-auto"
        ...
        >
          {data?.video_url ? (
            ...
            <div className="w-full h-full flex flex-col items-center justify-center gap-3 text-white/5 bg-[#080B12]">
              <Video className="w-12 h-12 mb-2" />
              <span className="uppercase tracking-[0.5em] text-[10px] font-black">Link Restricted</span>
            </div>
          )}

          {/* Premium Player Overlay */}
          <div className={`absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-black/20 transition-opacity duration-700 flex flex-col justify-between p-4 sm:p-6 md:opacity-0 group-hover:opacity-100 ${showControls ? 'opacity-100' : 'opacity-0'}`}>
            <div className="flex items-center justify-between">
              <div className="px-3.5 py-1.5 bg-black/40 backdrop-blur-md border border-white/10 rounded-full flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse shadow-[0_0_10px_rgba(239,68,68,0.8)]" />
                <span className="text-[9px] sm:text-[10px] font-black tracking-widest text-white uppercase">High Fidelity Stream</span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3 mt-auto w-full">
              ...
              <div className="flex flex-col gap-1.5">
                <div
                  ref={seekbarRef}
                  className={`relative h-1.5 rounded-full cursor-pointer group/seek transition-all duration-150 ${isDragging ? 'h-2.5' : 'hover:h-2.5'}`}
                ...
                >
                  ...
                  {/* Draggable Thumb */}
                  <div
                    className={`absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-3.5 h-3.5 rounded-full bg-white shadow-[0_0_12px_rgba(0,255,128,0.9)] border-2 border-primary transition-all duration-100 ${isDragging ? 'scale-125' : 'scale-0 group-hover/seek:scale-100'}`}
                    style={{ left: `${(currentTime / (duration || 1)) * 100}%` }}
                  />
                </div>
                <div className="flex justify-between items-center text-[9px] font-mono font-black text-white/40 tracking-widest px-0.5">
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <button
                  ...
                  className="group relative w-9 h-9 sm:w-10 sm:h-10 shrink-0 rounded-full bg-black/40 backdrop-blur-md border border-white/10 flex flex-col items-center justify-center text-white/60 hover:text-primary hover:border-primary/40 hover:bg-primary/10 active:scale-90 transition-all outline-none"
                >
                  <RotateCcw className="w-4 h-4" />
                  <span className="absolute -bottom-0.5 text-[6px] font-black tracking-tight text-white/30 group-hover:text-primary/60">10</span>
                </button>

                {/* Play/Pause */}
                <button
                  onClick={togglePlay}
                  className="w-12 h-12 sm:w-14 sm:h-14 shrink-0 rounded-full bg-primary flex items-center justify-center text-black shadow-[0_0_30px_rgba(0,255,128,0.5)] hover:scale-105 active:scale-95 transition-all outline-none"
                >
                ...
                {/* Fullscreen */}
                <button
                  className="w-8 h-8 sm:w-9 sm:h-9 shrink-0 rounded-full bg-black/40 backdrop-blur-md border border-white/10 flex items-center justify-center text-white/40 hover:text-white hover:bg-white/10 active:scale-95 transition-all outline-none"
